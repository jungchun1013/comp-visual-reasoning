...
def main():
    ...
    ax.plot(range(12), ref, color=C_T, lw=S["linewidth"], marker=S["marker"],
            ms=S["markersize"], label="asked-about object")
    ax.plot(range(12), nonref, color=C_D, lw=S["linewidth"], marker=S["marker"],
            ms=S["markersize"], label="the other object")
    ...
    ax.set_title("c  The decoder then reads that object", fontsize=S["subplot_title_fontsize"], loc="left")
    names = ["asked-about\nobject", "other\nobject", "background"]
