...
def draw_design(out_path: Path, layer: int = 9):
    ...
    # block rows
    xsA, xendA = _block_row(ax, x0, yA, "clean run", layer)
    xsB, xendB = _block_row(ax, x0, yB, "corrupted run", layer)
    ax.text(x0 + 3.6, yA + 0.7, "ViT blocks (frozen ViT with cross-attention layers; the two runs differ only in the question)",
    ...
    ax.text(xsA[layer] - 0.6, 6.8, f"at the output of block {layer}, one group of patch tokens\n"
            "in the clean run is replaced by the corrupted run's values", fontsize=8, color="#e6550d", va="center", ha="right")
    ...
    ax.text(x0, 1.2, "controls: replacing from the clean run itself reproduces the baseline at every block (1.00); "
