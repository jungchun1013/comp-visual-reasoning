...
把這兩層主張放在一起，會浮現一個可被實驗釘死的因果問題：**當 grounding 把 CLEVR 上的
VQA 準確率從無語言 conditioning 的水準拉升到 92% 時，它真正貢獻的那一步是什麼？** 一種
可能是它補上了基底所缺的屬性資訊；另一種可能是資訊本來就在基底裡，grounding 只是補上了
「選對物體」這一步選取器（selector）。本節以三條證據腿構成一個「凝視三角（fixation
triangle）」來裁決此問題：（i）量測**未經任何語言 conditioning 的原始基底**是否已逐物體
編碼屬性；（ii）量測**移除 cross-attention 的訓練模型**在何處失敗；（iii）量測**完整
grounding 的訓練模型**在同一組刺激下是否穩健。三腿交會處即回答上述因果問題。

本節另收兩項延伸證據，用以界定主張的邊界：一項 T2I 模型上的 zero-shot 檢驗，回應「此機制
是否只是 CLEVR 訓練的 artifact」的質疑；以及一則本地 Flamingo 基線的現況說明。

## 證據腿一：原始基底已逐物體編碼屬性（E8，raw-backbone probing）
...
換言之，基底缺的不是資訊，而是「用哪個物體」的選取——而選取正是後兩條腿所檢驗的。

## 證據腿二：移除 cross-attention 後，失敗是「選錯物體」而非「編碼遺失」（E7 + E5，−CA 消融）
...
一個新加入的誘餌物體，就能奪走 −CA 模型 **24–59%** 的答案。
...
只是不是被描述的那個。屬性編碼完好，斷掉的是選取。此結論與腿一一致：資訊在，選取器不在。

## 證據腿三：完整 grounding 使選取穩健（E7，主模型與 legacy SteerViT）

**方法。** 對完整 grounding 的訓練模型 `clevr_dinov2_concat_decoder1l_scratch_s42`（+CA），
跑與腿二**完全相同**的 E7 刺激（每屬性 100 對，答案不變性經 program 執行驗證）。為檢驗
...
同一個能奪走 −CA 模型 24–59% 答案的誘餌，只能捕獲完整模型 **≤6%** 的答案；準確率移動
...
因此本報告以 `hallucination_rate`（誘餌捕獲率）作為三角的主軸讀數（0–7% vs 24–59% 的
因果對比），並以 `bait_share_of_errors` 作為「錯誤確為誘餌形狀」的**形狀確認**輔助指標。
（此度量選用與 A1.3 的字面登記略有出入，見§限制。）

## 三角綜合：grounding 的因果貢獻是 Binding／選取這一步

三條腿在同一個因果問題上交會：

| 三角頂點 | 證據 | 讀法 |
|---|---|---|
| 原始基底（E8，腿一）| 逐物體可解碼率 0.92–1.00，四骨幹 | 資訊在，無選取器 |
| −CA 訓練（E5/E7 nogca，腿二）| 誘餌奪走 24–59%；100% 的錯誤顏色為場景內他物之值 | 選取斷，編碼完好 |
| +CA 訓練（E7，腿三）| 誘餌僅奪走 0–7%（主模型與 legacy SteerViT 皆然）| 選取運作 |

三腿合起來把因果貢獻釘死在**單一步**：基底免費供給逐物體屬性（腿一）；拿掉語言
conditioning，屬性仍在但讀出讀錯物體（腿二）；補回完整 grounding，凝視變得穩健（腿三）。
資訊的有無在三腿間不變，唯一隨 GCA 的有無而翻轉的是「讀出看哪個物體」。因此
**grounding 的因果貢獻正是 Binding／選取這一步**——語言 conditioning 挑選讀出所面對的
物體，而非補上任何缺失的屬性資訊。這同時關閉了 A1→A2 的論證弧：A1 動機所訴諸的「多物體
...
cross-attention 中、於任何受測 timestep 的問題 prompt 下浮現；本文所量測的 grounding
...
checkpoint 確認其僅具定性地位；Flamingo 的 E7 腿等待重訓後方能納入。Artifact：
`add_object_eval_clevr_flamingo_dinov2_early_s42_fixed.json`。

## 限制

1. **單一 seed。** 三角各腿均為 s42 單 seed；size 屬性的小幅移動（如主模型 E7 size
   acc +4）落在 n=100 的雜訊範圍內，不宜過度解讀。

2. **凝視三角的正向腿限於 direct query。** 腿三的穩健性結論針對 direct 屬性查詢；多物體
   場景中殘餘的瓶頸（集合枚舉、雙集合基數比較）不在本三角的裁決範圍，屬失敗模式分析
   （E5）另議。
...
4. **度量選用與預登記的出入。** A1.3 預先登記以 `bait_share_of_errors` 為頭條指標，本
   報告因該指標在錯誤稀有時不穩定，改以 `hallucination_rate` 為三角主軸、`bait_share`
   為形狀確認。此為對預登記字面的偏離，特此聲明（見證據腿三「度量選用說明」）。

5. **Flamingo 腿缺位。** 本地 Flamingo 基線因 4/16-epoch 訓練不足、預測退化至 chance
